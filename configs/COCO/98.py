...
from hanser.transform.detection import random_hflip, resize, pad_to
...
from hanser.train.lr_schedule import MultiStepLR
...
def transform(example, training=True):
    ...
    image, objects = pad_to(image, objects, output_size, mode='random' if training else 'corner')
...
lr_schedule = MultiStepLR(
    base_lr * mul, steps_per_epoch, milestones=[16, 22], gamma=0.1,
    warmup_min_lr=base_lr, warmup_steps=500)
...
learner = SuperLearner(
    ...
    work_dir=f"./drive/MyDrive/models/COCO-Detection/98")
