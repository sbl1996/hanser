...
def transform(example, output_size=(HEIGHT, WIDTH), training=True):
    ...
    if training:
        image = random_resize(image, output_size, ratio_range=(0.1, 2.0))
...
base_lr = 0.0002
epochs = 50
lr_schedule = CosineLR(base_lr * mul, steps_per_epoch, epochs,
                       min_lr=base_lr * mul * 0.05, warmup_min_lr=0, warmup_epoch=1)
...
learner = SuperLearner(
    ...
    work_dir=f"./drive/MyDrive/models/COCO-Detection/107")
...
learner.fit(
    ...
    local_eval_freq=[(0, 4), (40, 2)], callbacks=[EMA(0.9998, num_updates=optimizer.iterations)],
)