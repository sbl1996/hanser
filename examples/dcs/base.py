...
from hanser.models.layers import Conv2d, Linear, Act
...
def Num2Bit(Num, B):
    ...
    def integer2bit(integer, num_bits=B * 2):
        ...
        out = tf.expand_dims(integer, -1) // tf.constant([128, 64, 32, 16, 8, 4, 2, 1], tf.int32)
        return (out - (out % 1)) % 2
...
def Bit2Num(Bit, B):
    ...
    num = tf.reduce_sum(Bit_ * tf.constant([8, 4, 2, 1], tf.float32), -1)
    return num
...
def QuantizationOp(x, B):
    ...
    result = tf.cast((tf.round(x * 16 - 0.5)), dtype=tf.float32)
...
class CRBlock(Layer):
    ...
    def __init__(self, channels):
        ...
        self.path2 = Sequential([
            Conv2d(channels, channels, kernel_size=(1, 5), norm='def', act='def'),
            Conv2d(channels, channels, kernel_size=(5, 1), norm='def'),
        ])
        self.conv = Conv2d(channels * 2, 32, 1, norm='def')
        self.act = Act()
