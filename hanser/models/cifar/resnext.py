...
class ResNeXt(Model):
    ...
    def __init__(self, depth, cardinality, base_width, num_classes=10):
        super().__init__()
        layers = [(depth - 2) // 9] * 3
    ...
    def _make_layer(self, in_channels, channels, blocks, stride, cardinality, base_width):
        layers = [Bottleneck(in_channels, channels, stride, cardinality, base_width)]
        out_channels = channels * 4
        for i in range(1, blocks):
            layers.append(Bottleneck(out_channels, channels, 1, cardinality, base_width))
