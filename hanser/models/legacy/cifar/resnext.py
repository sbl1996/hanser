...
def bottleneck(x, channels=16, strides=1, conv_shortcut=False, name='block'):

    identity = x

    x = conv_block(x, channels, kernel_size=1, strides=1,
                   batch_normalization=True, activation=True, name=name + "/conv1")
    x = conv_block(x, channels, kernel_size=3, strides=strides,
                   batch_normalization=True, activation=True, name=name + "/conv2")
    x = conv_block(x, channels * 4, kernel_size=1, strides=1,
                   batch_normalization=True, activation=True, name=name + "/conv3")

    if conv_shortcut:
        identity = conv_block(identity, channels * 4, kernel_size=1, strides=strides,
                              batch_normalization=True, name=name + "/shortcut")
...
def stack(x, block, num_blocks, channels, strides, name='stage'):
    x = block(x, channels, strides=strides, conv_shortcut=True, name=name + "/block1")
    for i in range(num_blocks - 1):
        x = block(x, channels, strides=1, conv_shortcut=False, name=name + "/block" + str(i + 2))
    return x


def resnet(input_shape=(32, 32, 3), channels=16, depth=110, block='bottleneck', num_classes=10):
    if block == 'bottleneck':
        block = bottleneck
    ...
    x = stack(x, block, num_blocks, channels * 1, strides=1, name='stage1')
    x = stack(x, block, num_blocks, channels * 2, strides=2, name='stage2')
    x = stack(x, block, num_blocks, channels * 4, strides=2, name='stage3')
