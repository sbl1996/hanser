...
class ShuffleNetV2(Model):
    def __init__(self, stages_repeats, stages_out_channels, include_final_conv=False):
        super().__init__()
        ...
        if self.include_final_conv:
            ...
            self.final_conv = Conv2d(in_channels, out_channels, kernel_size=1,
                                norm='def', act='def')
            self.feat_channels = stages_out_channels[:3] + [stages_out_channels[-1]]
        else:
            self.feat_channels = stages_out_channels[:4]
